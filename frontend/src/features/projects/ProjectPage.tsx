...
export function ProjectPage(): React.ReactElement {
  ...
  const remove = useMutation({
    mutationFn: (document: DocumentRecord) => api.deleteDocument(projectId, document.id),
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["documents", projectId] }),
  });
  ...
  return <main className="app-shell"><header className="topbar"><Link to="/projects" className="brand">RFP <em>Lens</em></Link><Link to="/projects" className="text-button">프로젝트 목록</Link></header>
    ...
    <section className="document-history" aria-labelledby="document-history-title"><div className="section-heading"><div><p className="eyebrow">DOCUMENTS</p><h2 id="document-history-title">업로드한 공고문</h2></div><div>{documents.data?.some((document) => ["review_required", "completed", "partial"].includes(document.state)) && <><Link className="text-button" to={`/projects/${projectId}/overview`}>최종 분석 결과</Link><Link className="text-button" to={`/projects/${projectId}/review`}>요구사항 검토</Link><Link className="text-button" to={`/projects/${projectId}/compliance`}>컴플라이언스 표</Link></>}{documents.isFetching && <span className="muted">상태 갱신 중</span>}</div></div>
      {documents.isLoading && <p>문서 목록을 불러오는 중…</p>}
      {documents.isError && <p className="form-error" role="alert">문서 목록을 불러오지 못했습니다.</p>}
      <div className="document-list">{documents.data?.map((document) => <article key={document.id} className="document-row"><div><h3>{document.original_name}</h3><p className="muted">{document.media_type === "application/pdf" ? "PDF" : "HWPX"} · {document.block_count}개 블록</p></div><ProcessingStatus document={document} onRetry={(entry) => process.mutate(entry)} isRetrying={process.isPending && process.variables?.id === document.id} /><button className="text-button danger" onClick={() => remove.mutate(document)} disabled={remove.isPending && remove.variables?.id === document.id}>삭제</button></article>)}</div>
