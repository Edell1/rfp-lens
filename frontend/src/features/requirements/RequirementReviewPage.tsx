...
export function RequirementReviewPage(): React.ReactElement {
  ...
  const selected = requirements.data?.find((item) => item.id === (selectedId ?? requestedRequirementId)) ?? requirements.data?.[0];

  return <main className="app-shell"><header className="topbar"><Link to={`/projects/${projectId}`} className="brand">RFP <em>Lens</em></Link><div><Link to={`/projects/${projectId}/overview`} className="text-button">최종 분석 결과</Link><Link to={`/projects/${projectId}/compliance`} className="text-button">컴플라이언스 표</Link></div></header>
