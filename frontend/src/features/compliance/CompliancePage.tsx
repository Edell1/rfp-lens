...
export function CompliancePage(): React.ReactElement {
  ...
  return <main className="app-shell"><header className="topbar"><Link to={`/projects/${projectId}`} className="brand">RFP <em>Lens</em></Link><div><Link to={`/projects/${projectId}/overview`} className="text-button">최종 분석 결과</Link><Link to={`/projects/${projectId}/review`} className="text-button">요구사항 검토</Link></div></header>
